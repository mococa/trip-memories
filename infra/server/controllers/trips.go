...
func (*controller) CreateTrip(res http.ResponseWriter, req *http.Request) {
	...
	// Create trip
	created_trip, err := tripsService.CreateTrip(trip.PartitionKey, &trip)
	if err != nil {
...
func (*controller) FindTrip(res http.ResponseWriter, req *http.Request) {
	...
	// Getting user_pk and trip_sk query params
	user_pk := req.URL.Query().Get("user_pk")
	trip_sk := req.URL.Query().Get("trip_sk")

	// Find trip
	trip, err := tripsService.FindTrip(user_pk, trip_sk)
	if err != nil {
		res.WriteHeader(http.StatusInternalServerError)
		res.Write([]byte(fmt.Sprintf(`{"error": "%s"}`, err.Error())))
		return
...
func (*controller) ListTrips(res http.ResponseWriter, req *http.Request) {
	...
	// Find trips
	trips, err := tripsService.ListTrips(user_pk)
	if err != nil {
		res.WriteHeader(http.StatusInternalServerError)
		res.Write([]byte(fmt.Sprintf(`{"error": "%s"}`, err.Error())))
		return
